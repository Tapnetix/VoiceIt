...
PYGATE
                            ...
                            # ─── OpenAPI client drift gate (S16) ─────────────
                            # The committed JS client at app/src/lib/api/ is
                            # generated by scripts/generate-api.sh from the
                            # live backend's /openapi.json. Without this gate
                            # the generated client and backend drift silently:
                            # mocks against the (old) generated types stay
                            # green while the integrated call breaks.
                            #
                            # Re-runs the generator and fails if the committed
                            # client diverges from what the live backend emits.
                            export PATH="$HOME/.bun/bin:$PATH"
                            ...
                            bun --version
                            bun install --frozen-lockfile
                            # Boot backend ourselves so generate-api.sh's
                            # "already running" path is taken (its self-boot
                            # uses `backend.main:app` which is correct, but
                            # explicit boot here makes the lifecycle easier
                            # to trap for cleanup on failure).
                            backend/venv/bin/uvicorn backend.main:app --port 17493 \
                                > /tmp/uvicorn-drift.log 2>&1 &
                            UVICORN_PID=$!
                            ...
                            curl -s http://localhost:17493/openapi.json > /dev/null \
                                || { echo "::error::backend failed to boot for drift gate"; tail -50 /tmp/uvicorn-drift.log; exit 1; }
                            ./scripts/generate-api.sh
                            kill $UVICORN_PID 2>/dev/null || true
                            wait $UVICORN_PID 2>/dev/null || true
                            trap - EXIT
                            # Fail the build if the regenerated client differs
                            # from what's committed. Engineer fix: locally run
                            # `just generate-api` and commit the changes.
                            if ! git diff --quiet -- app/src/lib/api/; then
                                echo "::error::OpenAPI client drift detected — app/src/lib/api/ is out of sync with the backend's /openapi.json"
                                git diff --stat -- app/src/lib/api/
                                exit 1
                            fi
                            echo "OpenAPI client drift gate: PASS"
                        '''
                    }
                }
        ...
        // Internal-release pattern (mirrors PockeoR Jenkinsfile):
        // every push produces installer artifacts. macOS signing is best-effort
        // — Developer ID + notarization fire when the corresponding env vars
        // are set, otherwise ad-hoc signing keeps Gatekeeper happy for the
        // internal-distribution flow. Public-release work (notarized .dmgs,
        // GitHub draft releases, updater artifacts) is deferred until the
        // tauri-signing-key / Apple credentials are provisioned.
        stage('Build') {
            failFast false
            parallel {
